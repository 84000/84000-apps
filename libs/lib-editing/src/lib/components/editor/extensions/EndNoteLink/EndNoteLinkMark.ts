...
/**
 * The transaction that links the selection in `state` to an endnote, or null
 * when the schema has no endnote links.
 *
 * Only the end of the selection matters, since the marker renders there. A
 * link already ending there gets the note added to it; one continuing past it
 * is split.
 */
export const endNoteLinkTransaction = (
  state: EditorState,
  endNote: string,
  label: string | undefined,
  markName = 'endNoteLink',
): Transaction | null => {
  ...
  addCommands() {
    return {
      setEndNoteLink:
        (endNote, label) =>
        ({ state, dispatch }) => {
          const tr = endNoteLinkTransaction(state, endNote, label, this.name);
          if (!tr) return false;
