...
- Searches read the **published** text by default, which is the right default: what binds a translator is the house rendering as published, not a passage still being revised. `source: "draft"` reaches the editorial copy and needs `editor.read`; pass it only when someone is asking about work in progress, and say that you did — a draft quotation is not yet the library's word.
